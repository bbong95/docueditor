...
export type AgentType = 'businessman' | 'casual' | 'spacesuit' | 'detective';
...
export default function LegoAgent({
    position = [0, 0, 0],
    color,
    name,
    role: _role,
    ...
    return (
        <group position={position} ref={groupRef} onClick={() => console.log(`Agent ${name} clicked`)}>
            {/* Self-contained Lighting for Distributed View */}
            <hemisphereLight intensity={0.6} groundColor="#444444" />
            <pointLight position={[5, 10, 5]} intensity={1.5} castShadow />
            <ambientLight intensity={0.5} />

            <Float speed={4} rotationIntensity={0.5} floatIntensity={1}>
                <group scale={[scale, scale, scale]} onPointerOver={() => _setHovered(true)} onPointerOut={() => _setHovered(false)}>
            ...
            {showLabel && (
                ...
                >
                    <div style={{
                        ...
                        <p style={{ margin: 0, fontSize: '10px', fontWeight: 900, color: '#1e293b' }}>{name}</p>
                    </div>
                </Html>
            )}
        </group>
